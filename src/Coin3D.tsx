...
const CoinWithEmbeddedStars = forwardRef<CoinRef, CoinProps>(
  (
    {
      width = 400,
      height = 400,
    ...
    useEffect(() => {
      ...
      ? numTurns * 2 * Math.PI - rotationOffset
      : Infinity;    
      ...
      const animate = () => {
        ...
        if (autoStopAfterTurns) {
          const remaining = targetRotation - totalRotationRef.current;
          if (remaining > 0) {
            ...
            if (bounce) {
              const bounceFactor =
                remaining < decelerationThreshold
                  ? remaining / decelerationThreshold
                  : 1;
              group.position.y =
    ...
    // Экспонируем функцию перезапуска вращения через ref
